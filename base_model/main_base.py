...
from base_model.trainer_base import Trainer as Trainer_base
...
def backend_setting(option):
    ...
    if option.cuda:
        print(f'cuda setting on {option.gpu}')
...
def main():
    ...
    trainer = Trainer_base(option, logger)
    if option.data == 'utkface':
        ...
        if option.is_train:
            ...
            for val_type, val_loader in zip(valid_types, loaders):
                trainer._validate(val_loader, valid_type=val_type)
