...
class UBNet(nn.Module):
    def __init__(self,
        ...
        self.trans1 = nn.Sequential(
            nn.Conv2d(64, self.mid_ch, kernel_size = 1),
            nn.LeakyReLU(0.1)
        )
        ...
        self.trans3 = nn.Sequential(
            nn.Conv2d(128, self.mid_ch, kernel_size=1),
            nn.LeakyReLU(0.1)
        )
        self.trans4 = nn.Sequential(
            nn.Conv2d(256, self.mid_ch, kernel_size = 1),
            nn.LeakyReLU(0.1)
        )
        self.trans5 = nn.Sequential(
            nn.Conv2d(512, self.mid_ch, kernel_size = 1),
            nn.LeakyReLU(0.1)
        )
        self.reduction = CodeReduction(c_dim = self.mid_ch*5,  feat_hw = 7, blocks = 5)  #let feat_hw = width

        self.fc1 = nn.Linear(self.mid_ch, self.num_classes)
        self.fc2 = nn.Linear(self.mid_ch, self.num_classes)
        self.fc3 = nn.Linear(self.mid_ch, self.num_classes)
        self.fc4 = nn.Linear(self.mid_ch, self.num_classes)
        self.fc5 = nn.Linear(self.mid_ch, self.num_classes)
